...
def list_view(request):
    ...
    if class_filter is None:
        class_filter = '*'
    pupils_list = PupilsService.get_pupils_list(sort_order, filter, class_filter)
    ...
    return {'project': 'gdpr_permissions', 'pupils': pupils_list, 'capability_list': capability_list,
            'capabilities_nice':capability_dict, 'classes_dict':classes_dict, 'filter':filter, 'class_filter': class_filter}
...
def edit_pupil(request):
    ...
    return {'project': 'gdpr_permissions', 'pupil': pupil_info, 'capability_list': capability_list, 'capability_current': capability_current}
...
def list_2(request):
    pupil_list = PupilsService.pupil_join_query()
    print(f'returned pupil list from list_2: {pupil_list}')
    return {'project':'gdpr_permissions'}
